...
constexpr char const* kPluginName = "GroupPoints";
constexpr char const* kPluginVersion = "1";

class GroupPointsPlugin final : public nvinfer1::IPluginV2DynamicExt {
public:
    GroupPointsPlugin() = default;

    GroupPointsPlugin(void const* serial_data, size_t serial_length) {
        (void)serial_data;
        (void)serial_length;
    }

    nvinfer1::IPluginV2DynamicExt* clone() const noexcept override {
        auto* plugin = new GroupPointsPlugin();
    ...
    int32_t enqueue(
        ...
        const int nsample = input_desc[1].dims.d[2];
...
class GroupPointsPluginCreator final : public nvinfer1::IPluginCreator {
public:
    GroupPointsPluginCreator() {
    ...
    nvinfer1::IPluginV2* deserializePlugin(
        ...
        (void)name;
        auto* plugin = new GroupPointsPlugin(serial_data, serial_length);
        plugin->setPluginNamespace(namespace_.c_str());
        return plugin;
    }
...
REGISTER_TENSORRT_PLUGIN(GroupPointsPluginCreator);
