...
def parse_args():
    parser = argparse.ArgumentParser(description='IASSD::GroupPoints ORT custom op 단위 테스트')
    ...
    parser.add_argument('--npoint', type=int, default=64, help='group할 query center 수')
    parser.add_argument('--nsample', type=int, default=16, help='각 query center의 sample 수')
    parser.add_argument('--seed', type=int, default=8192, help='입력 생성 seed')
    parser.add_argument('--device', type=str, default='cuda', choices=['cuda'], help='현재 custom op는 CUDA EP만 지원')
    return parser.parse_args()


def make_group_model(path, batch_size, channels, num_points, npoint, nsample):
    graph = helper.make_graph(
        ...
        outputs=[
            helper.make_tensor_value_info('output', TensorProto.FLOAT, [batch_size, channels, npoint, nsample])
        ],
    )
    model = helper.make_model(
        ...
        producer_name='iassd_ort_group_test',
    )
...
def make_inputs(args):
    ...
    idx_np = rng.integers(
        ...
        size=(args.batch_size, args.npoint, args.nsample),
        dtype=np.int32,
    )
    return features_np, idx_np


def run_torch_group(features_np, idx_np):
    features = torch.from_numpy(features_np).cuda().contiguous()
    idx = torch.from_numpy(idx_np).cuda().contiguous()
    with torch.no_grad():
        output = pointnet2_utils.grouping_operation(features, idx)
    torch.cuda.synchronize()
    return output.cpu().numpy()


def run_ort_group(model_path, library_path, features_np, idx_np):
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        model_path = Path(temp_dir) / 'iassd_group_test.onnx'
        make_group_model(
            model_path,
            args.batch_size,
            args.channels,
        ...
        torch_output = run_torch_group(features_np, idx_np)
        ort_output = run_ort_group(model_path, library_path, features_np, idx_np)
    ...
    if not np.array_equal(torch_output, ort_output):
        ...
        raise AssertionError(f'Group output mismatch: diff_count={diff_count}, max_abs_diff={max_abs_diff}')

    print('IASSD::GroupPoints ORT custom op 테스트 통과')
    print(
